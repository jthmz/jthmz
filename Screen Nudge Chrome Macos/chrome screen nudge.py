...
# USER INPUT 
##############################################################
# Update Application Information for Google Chrome
...
if ! [[ $bundleid =~ ^[a-zA-Z0-9]+[-.](.*) ]]; then
  LOGGING "--- Could not find a valid bundleid for $appName at $appPath!"
  exit 1
fi
...
#Check to see if app is installed and if ScreenCapture is approved.
  ...
  LOGGING "--- Could not find a valid PPPC Profile for $appName allowing Standard User to Approve."
  exit 1
else
